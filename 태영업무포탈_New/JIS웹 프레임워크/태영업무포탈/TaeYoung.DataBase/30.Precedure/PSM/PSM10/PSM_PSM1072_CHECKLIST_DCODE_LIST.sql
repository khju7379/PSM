...
DROP PROCEDURE PSSCMLIB.PSM_PSM1072_CHECKLIST_DCODE_LIST;
  
CREATE PROCEDURE PSSCMLIB.PSM_PSM1072_CHECKLIST_DCODE_LIST ( 
	...
	LANGUAGE SQL 
	SPECIFIC PSSCMLIB.PSM_PSM1072_CHECKLIST_DCODE_LIST 
	NOT DETERMINISTIC 
	MODIFIES SQL DATA 
...
MAIN : BEGIN  -- 실행부 
      LIST : BEGIN  -- 리스트 
		DECLARE REFCURSOR CURSOR WITH RETURN FOR  -- 커서생성 
			...
			SELECT
			ROW_NUMBER() OVER(ORDER BY CIITEMNUM) AS ROWNO,
			CISBCODE,
			CIGRCODE,
			CIGRSEQ,
			CIITEMNUM,
