--  Generate SQL 
--  Version:                   	V6R1M0 080215 
--  Generated on:              	18/10/16 08:29:28 
--  Relational Database:       	S65685C2 
--  Standards Option:          	DB2 i5/OS 

DROP PROCEDURE PSSCMLIB.PSM_SIGNCODE_LIST;
  
CREATE PROCEDURE PSSCMLIB.PSM_SIGNCODE_LIST ( 
	IN P_CURRENTPAGEINDEX INTEGER , 
	IN P_PAGESIZE INTEGER , 
	IN P_KBHANGL VARCHAR(20) 
	) 
	DYNAMIC RESULT SETS 2 
	LANGUAGE SQL 
	SPECIFIC PSSCMLIB.PSM_SIGNCODE_LIST 
	NOT DETERMINISTIC 
	MODIFIES SQL DATA 
	CALLED ON NULL INPUT 
	SET OPTION  ALWBLK = *ALLREAD , 
	ALWCPYDTA = *OPTIMIZE , 
	COMMIT = *NONE , 
	CLOSQLCSR = *ENDMOD , 
	DECRESULT = (31, 31, 00) , 
	DFTRDBCOL = *NONE , 
	DYNDFTCOL = *NO , 
	DYNUSRPRF = *USER , 
	SRTSEQ = *HEX   
	P1 : BEGIN  -- 시작 
	DECLARE P_STNUM INTEGER ; 
	DECLARE P_FNNUM INTEGER ; 
	DECLARE P_SQLSTRING VARCHAR ( 4000 ) ; 
	DECLARE P_SQLTOTALROWCOUNT VARCHAR ( 4000 ) ; 
	DECLARE P_TABLE_QUERY			VARCHAR ( 4000 ) ; 
	DECLARE P_COUNT_QUERY			VARCHAR ( 4000 ) ; 
  
PREV : BEGIN  -- 값 설정 
		SET P_STNUM = ( P_PAGESIZE * ( P_CURRENTPAGEINDEX - 1 ) ) + 1 ; 
        SET P_FNNUM = P_PAGESIZE * P_CURRENTPAGEINDEX ; 
	END PREV ; 
  
MAIN : BEGIN  -- 실행부 
      LIST : BEGIN  -- 리스트 
			DECLARE REFCURSOR CURSOR WITH RETURN FOR  
				 WITH ORIGINAL_DATA AS (
				 SELECT 
					ROW_NUMBER() OVER(ORDER BY KBSABUN) AS ROWNO,
					KBSABUN ,
					KBHANGL,
					KBJKCD,
					JKCODE.CDDESC1 AS JKDESC,
					TYSCMLIB.SF_GB_NEWORGID(KBSABUN, 'CODE', SUBSTR(CHAR(CURRENT_DATE),1,4)||SUBSTRING(CHAR(CURRENT_DATE),6,2)||SUBSTRING(CHAR(CURRENT_DATE),9,2)) AS KBBUSEO,
					TYSCMLIB.SF_GB_NEWORGID(KBSABUN, 'NAME', SUBSTR(CHAR(CURRENT_DATE),1,4)||SUBSTRING(CHAR(CURRENT_DATE),6,2)||SUBSTRING(CHAR(CURRENT_DATE),9,2)) AS KBBUSEONM,
					FILE_NAME,
					ATTACH_BYTE
				   FROM TYSCMLIB.INKIBNMF
				   LEFT OUTER JOIN PSSCMLIB.PSM_CMN_ATTACH
					ON    ATTACH_TYPE = 'SG'
				   AND    ATTACH_NO   = KBSABUN
				   LEFT OUTER JOIN TYSCMLIB.INCODEMF AS JKCODE
					ON            'JK' = JKCODE.CDINDEX
				   AND     KBJKCD = JKCODE.CDCODE
				   WHERE  KBBALCD   NOT IN ('900','560')
				    AND   KBHANGL LIKE '%'||P_KBHANGL||'%'
				)
				SELECT
				*
				FROM ORIGINAL_DATA
				WHERE ROWNO BETWEEN P_STNUM AND P_FNNUM
				ORDER BY ROWNO ASC ;

			OPEN REFCURSOR ; 

			END LIST ; 
  
			PAGING : BEGIN  -- 페이징 
					 DECLARE REFCURSOR2 CURSOR WITH RETURN FOR
					 WITH ORIGINAL_DATA AS (
						 SELECT 
							ROW_NUMBER() OVER(ORDER BY KBSABUN) AS ROWNO,
							KBSABUN ,
							KBHANGL,
							KBJKCD,
							JKCODE.CDDESC1 AS JKDESC,
							TYSCMLIB.SF_GB_NEWORGID(KBSABUN, 'CODE', SUBSTR(CHAR(CURRENT_DATE),1,4)||SUBSTRING(CHAR(CURRENT_DATE),6,2)||SUBSTRING(CHAR(CURRENT_DATE),9,2)) AS KBBUSEO,
							TYSCMLIB.SF_GB_NEWORGID(KBSABUN, 'NAME', SUBSTR(CHAR(CURRENT_DATE),1,4)||SUBSTRING(CHAR(CURRENT_DATE),6,2)||SUBSTRING(CHAR(CURRENT_DATE),9,2)) AS KBBUSEONM,
							FILE_NAME,
							ATTACH_BYTE
						   FROM TYSCMLIB.INKIBNMF
						   LEFT OUTER JOIN PSSCMLIB.PSM_CMN_ATTACH
							ON    ATTACH_TYPE = 'SG'
						   AND    ATTACH_NO   = KBSABUN
						   LEFT OUTER JOIN TYSCMLIB.INCODEMF AS JKCODE
							ON            'JK' = JKCODE.CDINDEX
						   AND     KBJKCD = JKCODE.CDCODE
						   WHERE  KBBALCD   NOT IN ('900','560')
							AND   KBHANGL LIKE '%'||P_KBHANGL||'%'
						)
						SELECT COUNT(*) AS TOTALCOUNT
						FROM ORIGINAL_DATA;

					OPEN REFCURSOR2 ; 
			END PAGING ; 
END MAIN ; 
END  ;
