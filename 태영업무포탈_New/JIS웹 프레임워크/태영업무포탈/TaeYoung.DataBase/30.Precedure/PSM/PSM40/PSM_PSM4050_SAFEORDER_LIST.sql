DROP PROCEDURE PSSCMLIB.PSM_PSM4050_SAFEORDER_LIST;

CREATE PROCEDURE PSSCMLIB.PSM_PSM4050_SAFEORDER_LIST
( 
	IN P_SMWKTEAM     VARCHAR(6),
    IN P_SMWKDATE     VARCHAR(8),
    IN P_SMWKSEQ     VARCHAR(3),
...
P1 : BEGIN -- 시작
	...
	DECLARE REFCURSOR CURSOR WITH RETURN FOR						
            ...
            SELECT 
                '<img src="/Resources/Images/btn_print.gif" style="cursor:pointer" onclick="GridSafeDocClick(''' || SMWKTEAM || '-' || SMWKDATE || '-' || DIGITS ( SMWKSEQ ) || '-' || SMWKORAPPDATE || '-' || DIGITS ( SMWKORSEQ ) || ''');" />' AS SAFEDOCPRT ,                
                CASE WHEN  ( SELECT COUNT(*)  FROM PSSCMLIB.PSM_JSACHANGE_MASTER
                       		  WHERE JSMWKGUBN   = 'D'
	                       	    AND   JSMPOTEAM = SMWKTEAM
		                        AND   JSMPODATE = SMWKDATE
		                        AND   JSMPOSEQ  = SMWKSEQ
		                        AND   JSMDATE   = SMWKORAPPDATE ) > 0 THEN '<img src="/Resources/Images/btn_print.gif" style="cursor:pointer" onclick="GridJSADocClick(''' || SMWKTEAM || '-' || SMWKDATE || '-' || DIGITS ( SMWKSEQ ) || '-' || SMWKORAPPDATE || '-' || DIGITS ( SMWKORSEQ ) || ''');" />' 
                ...
                SMSYSTEMCODE2 , 
                LCT2 . FXC3DESC AS C3NAME2 , 
                SMSYSTEMCODE3 , 
                LCT3 . FXC3DESC AS C3NAME3 , 
                SMSYSTEMCODE4 , 
                LCT4 . FXC3DESC AS C3NAME4 , 
                SMSYSTEMCODE5 , 
                LCT5 . FXC3DESC AS C3NAME5 , 
                SMCONNCODE11 , 
                LCT11 . FXC3DESC AS C3NAME11 , 
                SMCONNCODE12 , 
                LCT12 . FXC3DESC AS C3NAME12 , 
                SMCONNCODE21 , 
                LCT21 . FXC3DESC AS C3NAME21 , 
                SMCONNCODE22 , 
                LCT22 . FXC3DESC AS C3NAME22 , 
                SMCONNCODE31 , 
                LCT31 . FXC3DESC AS C3NAME31 , 
                SMCONNCODE32 , 
                LCT32 . FXC3DESC AS C3NAME32 , 
                SMCONNCODE41 , 
                LCT41 . FXC3DESC AS C3NAME41 , 
                SMCONNCODE42 , 
                LCT42 . FXC3DESC AS C3NAME42 , 
                SMCONNCODE51 , 
                LCT51 . FXC3DESC AS C3NAME51 , 
                SMCONNCODE52 , 
                LCT52 . FXC3DESC AS C3NAME52 , 
                SMAREACODE1 , 
                ...
                SMSMAPPDATE , 
                SMSMAPPTIME , 
                CASE WHEN SMCOSABUN <> '' AND SMCOAPPDATE <> '' AND SMCOAPPTIME <> '' THEN 'Y' ELSE 'N' END AS SAFEPRINTGN , 
