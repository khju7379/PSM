...
DROP PROCEDURE PSSCMLIB.PSM_PSM4045_JSACHANGE_PRT_LIST ; 
  
CREATE PROCEDURE PSSCMLIB.PSM_PSM4045_JSACHANGE_PRT_LIST ( 
	...
	DECLARE V_OUT_MSG VARCHAR ( 100 ) ; 
MAIN : BEGIN  -- 실행부 

	CALL PSSCMLIB.PSM_PSM4045_JSACHANGE_PRT_CREATE(P_SESSION, P_JSMWKGUBN, P_JSMPOTEAM, P_JSMPODATE, P_JSMPOSEQ, P_JSMDATE, P_JSMMSEQ, P_JSMBLASS, P_JSMMLASS, P_JSMSLASS, P_JSMSEQ, P_SABUN, V_OUT_MSG);
...
LIST : BEGIN  -- 리스트 
		...
		ELSE '<img src="/Resources/Images/Icon/it_res.png" style="cursor:pointer" onclick="gridClick(''' || PRITEMSEQ || ''');" /> ' || PRITEMTEXT
		END  AS PRITEMTEXT,
		CASE WHEN PRITEMTEXT = '' THEN PRTOOLTEXT
		WHEN P_GUBUN <> 'S' THEN PRTOOLTEXT
		ELSE '<img src="/Resources/Images/Icon/it_res.png" style="cursor:pointer" onclick="gridClick(''' || PRITEMSEQ || ''');" /> ' || PRTOOLTEXT
		END  AS PRTOOLTEXT,
		
		 PRRSUBSEQ,
		CASE WHEN P_GUBUN <> 'S' THEN PRRISKTEXT
		ELSE '<img src="/Resources/Images/Icon/it_res.png" style="cursor:pointer" onclick="gridRiskClick(''' || PRITEMSEQ || '^' || PRRSUBSEQ || ''');" /> ' || PRRISKTEXT
		END AS PRRISKTEXT,
		PRESUBSEQ,
		CASE WHEN P_GUBUN <> 'S' THEN PREREFORMTEXT
		ELSE '<img src="/Resources/Images/Icon/it_res.png" style="cursor:pointer" onclick="gridReformClick(''' || PRITEMSEQ || '^' || PRESUBSEQ || ''');" /> ' || PREREFORMTEXT 
