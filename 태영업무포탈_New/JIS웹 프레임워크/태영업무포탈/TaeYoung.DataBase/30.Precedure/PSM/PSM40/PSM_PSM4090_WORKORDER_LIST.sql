...
DROP  PROCEDURE PSSCMLIB.PSM_PSM4090_WORKORDER_LIST;

CREATE PROCEDURE PSSCMLIB.PSM_PSM4090_WORKORDER_LIST ( 
	...
	DECLARE REFCURSOR CURSOR WITH RETURN FOR 
		...
		( CASE WHEN WK . WOSTATUS <> '5' AND (SELECT COUNT(*) FROM PSSCMLIB.PSM_EXAM_CHECK WHERE PEC.PECWOTEAM = WK.WOORTEAM AND  PEC.PECWODATE = WK.WOORDATE AND  PEC.PECWOSEQ = WK.WOSEQ) > 0 THEN '가동전 점검 중'
		...
		WHEN CAASKSTEPGN = '4' THEN '비상' ELSE '임시' END ) AS CAASKSTEPGN,
		( CASE WHEN WK . WOSTATUS <> '5' AND (SELECT COUNT(*) FROM PSSCMLIB.PSM_EXAM_CHECK WHERE PEC.PECWOTEAM = WK.WOORTEAM AND  PEC.PECWODATE = WK.WOORDATE AND  PEC.PECWOSEQ = WK.WOSEQ) > 0 THEN '가동전 점검 중'
		WHEN WK . WOSTATUS = '5' AND WK.WOAPPSTATUS = 'F' THEN '공사 완료' ELSE '변경검토서 완료' END) AS WOSTATUS,
		( CASE WHEN WK . WOSTATUS = '5' AND WK . WOAPPSTATUS = 'F' THEN '5'  -- 공사완료 
		ELSE '1' END ) AS WORKSTATUS  -- 진행중 
		FROM PSSCMLIB.PSM_WORKORDER AS WK
		LEFT OUTER JOIN PSSCMLIB.PSM_CHANGEASK_DOC AS CA
		...
		AND   CA.CAWODATE = WK.WOORDATE
		AND   CA.CAWOSEQ = WK.WOSEQ
		LEFT OUTER JOIN PSSCMLIB.PSM_CHANGEREV_DOC AS CR
		ON     CR.CRASKTEAM = CA.CAASKTEAM
		AND   CR.CRASKDATE = CA.CAASKDATE
		AND   CR.CRASKSEQ = CA.CAASKSEQ
		LEFT OUTER JOIN TYSCMLIB . ACFIXCLASS3 S1 
		ON S1 . FXC3SAUP || S1 . FXC3BCODE || DIGITS ( S1 . FXC3MCODE ) || DIGITS ( S1 . FXC3SCODE ) = WK.WOLOCATIONCODE1 
		LEFT OUTER JOIN PSSCMLIB.PSM_EXAM_CHECK AS PEC
		ON    PEC.PECWOTEAM = WK.WOORTEAM
