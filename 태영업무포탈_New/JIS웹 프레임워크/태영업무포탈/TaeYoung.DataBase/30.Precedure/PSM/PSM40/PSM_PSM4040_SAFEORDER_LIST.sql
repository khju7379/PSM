...
CREATE PROCEDURE PSSCMLIB.PSM_PSM4040_SAFEORDER_LIST ( 
	...
	LANGUAGE SQL 
	SPECIFIC PSSCMLIB.PSM_PSM4040_SAFEORDER_LIST 
	NOT DETERMINISTIC 
	MODIFIES SQL DATA 
	...
	DECLARE REFCURSOR CURSOR WITH RETURN FOR 
		...
		SELECT 
		( CASE WHEN (SELECT WOFINISHDATE FROM PSSCMLIB.PSM_WORKORDER AS B
			     WHERE A.SMWKTEAM = B.WOORTEAM
			     AND   A.SMWKDATE = B.WOORDATE
			     AND   A.SMWKSEQ  = B.WOSEQ
			     AND   B.WOFINISHDATE <> ''
			     AND   B.WOFINISHSABUN <> '')
			     <> '' THEN '' ELSE '<img src="/Resources/Images/icon_file.gif" style="cursor:pointer" onclick="GridSafeCopy(''' || SMWKTEAM || '-' || SMWKDATE || '-' || DIGITS ( SMWKSEQ ) || '-' || SMWKORAPPDATE || '-' || DIGITS ( SMWKORSEQ ) || ''');" />' END ) AS SAFECOPY , 
		( CASE WHEN SMCOSABUN <> '' AND SMCOAPPDATE <> '' AND SMCOAPPTIME <> '' THEN '<img src="/Resources/Images/btn_print.gif" style="cursor:pointer" onclick="GridSafeDocClick(''' || SMWKTEAM || '-' || SMWKDATE || '-' || DIGITS ( SMWKSEQ ) || '-' || SMWKORAPPDATE || '-' || DIGITS ( SMWKORSEQ ) || ''');" />' 
		ELSE '' END ) AS SAFEDOCPRT , 
		( CASE WHEN SMCOSABUN <> '' AND SMCOAPPDATE <> '' AND SMCOAPPTIME <> '' THEN '<img src="/Resources/Images/btn_print.gif" style="cursor:pointer" onclick="GridJSADocClick(''' || SMWKTEAM || '-' || SMWKDATE || '-' || DIGITS ( SMWKSEQ ) || '-' || SMWKORAPPDATE || '-' || DIGITS ( SMWKORSEQ ) || ''');" />' 
		...
		SMFSWKCLOSE		 
		FROM PSSCMLIB . PSM_SAFEORDER_MASTER AS A
