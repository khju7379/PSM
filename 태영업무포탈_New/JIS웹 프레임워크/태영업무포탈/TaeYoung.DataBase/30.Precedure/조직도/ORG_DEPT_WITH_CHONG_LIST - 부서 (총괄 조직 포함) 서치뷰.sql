-------------------------------------------------------------------------------------------
--
-- 프로시저명  : TYJINFWLIB.ORG_DEPT_WITH_CHONG_LIST
-- 작성자     : 김희섭
-- 작성일     : 2015-09-16
-- 설명       : 부서 서치뷰2
...
-------------------------------------------------------------------------------------------
CREATE PROCEDURE TYJINFWLIB.ORG_DEPT_WITH_CHONG_LIST 
(
...
P1: BEGIN
	...
	FROM(
		...
		WHERE ( COMPANYCODE = P_COMPANYCODE
           OR  COMPANYCODE IN (SELECT COMPANYCODE FROM TYJINFWLIB.ORG_COMPANY WHERE COMMON = '1') 
           )
          AND (A.DEPTCODE LIKE P_SEARCHCONDITION || '%' OR A.DEPTNAME LIKE P_SEARCHCONDITION || '%')
	) AS A;
