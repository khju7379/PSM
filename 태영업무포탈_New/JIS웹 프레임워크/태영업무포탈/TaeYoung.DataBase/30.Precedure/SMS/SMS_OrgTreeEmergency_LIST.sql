...
DROP PROCEDURE TYJINFWLIB.SMS_OrgTreeEmergency_LIST;
  
CREATE PROCEDURE TYJINFWLIB.SMS_OrgTreeEmergency_LIST ( 
	...
	IN P_ADDGROUP    VARCHAR(10)
	) 
	DYNAMIC RESULT SETS 1 
	LANGUAGE SQL 
	SPECIFIC TYJINFWLIB.SMS_OrgTreeEmergency_LIST 
	NOT DETERMINISTIC 
	MODIFIES SQL DATA 
		...
		WITH n ( level, ORG_CD, SDATE,  PRIOR_ORG_CD,  SEQ, T_LEVEL, ROWNUM) AS 
                   ( SELECT   1 AS level, 
                        ...
                        (CASE WHEN (LEVEL = '4' AND PNODE = 'S30000') THEN LEVEL-1
                                    WHEN SABUN IN ('0002-M','0123-M', '0383-M', '0034-M', '0280-M', '0441-M','0442-M') THEN LEVEL
                        ...
                        LEFT OUTER JOIN TYSCMLIB.PSM_SMSManager_ADDETAIL
                        ON       EMSABUN = ADDSABUN
                        WHERE EMYYMM =  CAST(   (SELECT MAX(EMYYMM ) FROM TYSCMLIB.EIORGMEMBERF )  AS VARCHAR(6))
                            AND EMENTER_CD  = 'TY'
                            AND EMORG_CD <> ''
                            AND EMJCCD != '245'   /* 부팀장 제외 */
                            -- AND  EMSABUN||EMORG_CD <>  '0054-MT10000'   /* 신성훈부장 운영팀장 제외 */
                            -- AND  EMSABUN||EMORG_CD <>  '0280-MT10100'   /* 조태홍상무 영업팀장 제외 */
                            ...
                            AND  ADDGROUP IN ( SELECT * FROM TABLE(TYSCMLIB.SF_GB_RevColRow(CAST(P_ADDGROUP AS VARCHAR(100)), ',')) AS InTable )
                        ) D1
                        WHERE LEVEL > 1
                        ) AS TEMP
