...
GW.OrgMap.Common.Contacts = function (callFrom, overlap, OutlookItemType) {
    GW.OrgMap.Common.Contacts.superclass.constructor.call(this);
...
Ext.extend(GW.OrgMap.Common.Contacts, GW.OrgMap.Common, {
...
Ext.extend(GW.OrgMap.Common.CustomMailGroup, GW.OrgMap.Common, {
    ...
    Render: function () {
        ...
        this.CMGCheck.on("selectionchange", function (sm) {
            ...
            if (group) {
                this.dsCMGMember.load({ params: { groupADCN: group.get("ADCommonName")} });
            }
        }, this);
        this.CompanyCombo.on("change", function (combo, item) {
        ...
        if (!this.AddCustomGroupWindow) {
            function onSubmit() {
                if (!groupNameField.validate()) {
                    ...
                    var groupOrder = form.items.get(2).getValue();
            ...
            var form = new Ext.FormPanel({
                ...
                //	scope:this
                //},
                items: [companyCombo, groupNameField, {
        ...
        if (!this.UpdateCustomGroupWindow) {
            function onUpdateSubmit() {
                if (!groupNameField.validate()) {
                    ...
                    var loginID = customGroup.get("CreatorLoginID");
                    var groupADCN = customGroup.get("ADCommonName");
    ...
    AddMemberToCustomGroup: function () {
        ...
        if (!this.MiniOrgMap) {
            ...
            var miniMainPanel = new Ext.Panel({
                ...
                border: false,
                bodyStyle: 'background: #FFFFFF none repeat scroll 0 0;',
                tbar: ['-', miniCompanyCombo, '-', '->', '-', searchBox, '-'],
        ...
        if (members && members.length > 0) {
			...
			function (btn) {
			    if (btn == "yes") {
			        ...
			        var groupMembers = "";
			        var delimiter = "$";
			        Ext.each(members, function (record, index, allItems) { groupMembers += record.get('sAMAccountName') + delimiter; }, this);
    ...
    OnMiniOk: function () {
        ...
        if (this.dsMiniResult.getCount() > 0) {
            ...
            this.dsMiniResult.each(function (record) {
                var userAccountID = record.get('AccountId');
