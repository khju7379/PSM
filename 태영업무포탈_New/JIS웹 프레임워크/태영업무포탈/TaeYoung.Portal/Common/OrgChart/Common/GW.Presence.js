...
GW.Presence = function (el, name) {
    this.type = "GW.Presence";
    this.name = name;
    this.status = null;
    this.imgEl = el;
...
GW.Presence.idSeed = 0;
GW.Presence.BLANK_IMAGE_URL = "common/presence.image/BLANK.GIF";

// singleton
GW.Presence.Templates = new Ext.Template(
	'<span id="{id}" class="gw-presence">',
		'<img class="gw-presence-icon gw-presence-unk" onload="GW.NameControl.Bind(this, \'{name}\');" src="' + GW.Presence.BLANK_IMAGE_URL + '" />',
	'</span>'
);

GW.Presence.prototype = {
...
GW.PresenceMgr.prototype = {
    ...
    Bind: function (el, name) {
        ...
        var presence = new GW.Presence(el, name);
        if (this.isCanUse) {
    ...
    toHTMLString: function (name, id) {
        var id = id || "gw-presence-" + (++GW.Presence.idSeed);
        return GW.Presence.Templates.apply({ id: id, name: name });
    }
};
