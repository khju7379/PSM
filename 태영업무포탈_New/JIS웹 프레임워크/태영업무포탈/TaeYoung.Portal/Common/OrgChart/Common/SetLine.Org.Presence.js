...
GW.SetLine.Org.Presence = function(el, name) {
    this.type = "GW.SetLine.Org.Presence";
	this.name = name;
	this.status = null;
	this.imgEl = el;
...
GW.SetLine.Org.Presence.idSeed = 0;
GW.SetLine.Org.Presence.BLANK_IMAGE_URL = "Common/presence.image/BLANK.GIF";

// singleton
GW.SetLine.Org.Presence.Templates = new Ext.Template(
	'<span id="{id}" class="gw-presence">',
		'<img class="gw-presence-icon gw-presence-unk" onload="GW.NameControl.Bind(this, \'{name}\');" src="' + GW.SetLine.Org.Presence.BLANK_IMAGE_URL + '" />',
	'</span>'
);

GW.SetLine.Org.Presence.prototype = {
...
GW.PresenceMgr.prototype = {
	...
	Bind : function(el, name) {
		...
		var presence = new GW.SetLine.Org.Presence(el, name);
		if(this.isCanUse)
		{
	...
	toHTMLString : function(name, id) {
		var id = id || "gw-presence-" + (++GW.SetLine.Org.Presence.idSeed);
		return GW.SetLine.Org.Presence.Templates.apply({id:id, name:name});
	}
};
