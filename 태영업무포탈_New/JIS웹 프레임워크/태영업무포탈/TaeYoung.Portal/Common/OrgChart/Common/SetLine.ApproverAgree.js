/*
* GW.SetLine.ApproverAgree.js
* 결재자/협조자
*/
GW.SetLine.ApproverAgree = function () {

    //GW.SetLine.ApproverAgree.superclass.constructor.call(this);
...
Ext.extend(GW.SetLine.ApproverAgree, GW.SetLine.Common, {
    ...
    On_ApproverAgree_DbClickItem: function (record, index, allItems) {
                    ...
                    switch (history_node.childNodes[i].nodeName) {
                        case "Approver":
                            ...
                            if (foundItem != -1) {
                                ...
                                dsApprover.data.items[foundItem].set('CompanyCode', history_node.childNodes[i].getAttribute("CompanyCode"));
                            ...
                            if (this.CRowCount > 0) {
                                ...
                                if (foundItem != -1) {
                                    ...
                                    dsCharge.data.items[foundItem].set('CompanyCode', history_node.childNodes[i].getAttribute("CompanyCode"));
    ...
    DragDropSetting: function () {
        ...
        function HandlerApproverGridDrop(ddSource, dsResult) {
            ...
            Ext.each(ddSource.dragData.selections, function (record, index, allItems) {
                if (record.data.Type != "GROUP") {
                    ...
                    if (foundItem == -1) {
                        ...
                        if (dsResult.getSelectionModel().getSelections().length > 0) {
                            ...
                            Ext.each(dsResult.getSelectionModel().getSelections(), function (trecord, tindex, tallItems) {
                                ...
                                if (approver != null) {
                                    ...
                                    if (GW.ApproverAgree.CheckUserId) {
                                        if (approver.getAttribute("LoginID") == GW.ApproverAgree.CurrentUser && approver.getAttribute("TaskID") == GW.ApproverAgree.CurrentUserTaskID) {
                                ...
                                //협조탭 Enable처리
                                if (is_agree_multi) GW.ApproverAgree.EnableAgreeTab(trecord.get('TaskId'), true);
                            }, this);
                        }
                        else {
                            return;
                            ...
                            for (var j = dsResult.store.data.items.length - 1; j > -1; j--) {
                                if (dsResult.store.data.items[j].data.EMPID == undefined || dsResult.store.data.items[j].data.EMPID == "") {
                                    ...
                                    //협조탭 Enable처리
                                    if (is_agree_multi) GW.ApproverAgree.EnableAgreeTab(dsResult.store.data.items[j].get('TaskId'), true);
                                    break;
                                }
                            }
                        }
                        ...
                        GW.ApproverAgree.WarningMessageBox(GetResource("ApproveValidationCheckMsg", args));
                    }
                    //Activate Tab선택
                    if (is_agree_multi) GW.ApproverAgree.ActivateAgreeTab();
                }
                else {
                    isGroup = true;
                }
            ...
            if (isGroup) {
                GW.ApproverAgree.WarningMessageBox(GetResource("ApproverNotSetGroupMsg"));
            }

            if (strNoEmpIDUserList.length > 1) {
                GW.ApproverAgree.WarningMessageBox(strNoEmpIDUserList + GetResource("NoInfoAddMsg"));
            }
            GW.ApproverAgree.AllSelectionClear();
        }
...
//                        var foundItem = -1
            ...
            //협조 탭 타이틀 변경
            //var agreeId = ApproverAgree.TabAgreeContainer.activeTab.id
            // ApproverAgree.ChangeTabTitle(agreeId, dsResult);
    ...
    HandlerAddAgree: function (dataGrid, dsResult) {
        ...
        Ext.each(this.CurrentGridSet.Select.getSelectionModel().getSelections(), function (record, index, allItems) {
            ...
            if (record.data.Type != "GROUP") {
                ...
                // EmpId 유일키
//                if (is_overlap)
//                    foundItem = dsResult.find('EmpID', record.data.EmpID);
//                else
//                    foundItem = -1;
    ...
    ChangeTabTitle: function (id, ds) {
        ...
        switch (id) {
            case 'C101':
                ...
                strTitle = String.format(GetResource("ApproverAgreeTabDraft"), ds.data.getCount());
                break;

            case 'E101':
                //strTitle = String.format('결재1({0})', ds.data.getCount())
                strTitle = String.format(GetResource("ApproverAgreeTabApp1"), ds.data.getCount());
                break;

            case 'E102':
                //strTitle = String.format('결재2({0})', ds.data.getCount())
                strTitle = String.format(GetResource("ApproverAgreeTabApp2"), ds.data.getCount());
                break;

            case 'E103':
                //strTitle = String.format('결재3({0})', ds.data.getCount())
                strTitle = String.format(GetResource("ApproverAgreeTabApp3"), ds.data.getCount());
                break;

            case 'E104':
                //strTitle = String.format('결재4({0})', ds.data.getCount())
                strTitle = String.format(GetResource("ApproverAgreeTabApp4"), ds.data.getCount());
                break;

            case 'E105':
                //strTitle = String.format('결재5({0})', ds.data.getCount())
                strTitle = String.format(GetResource("ApproverAgreeTabApp5"), ds.data.getCount());
                break;

            case 'E106':
                //strTitle = String.format('결재5({0})', ds.data.getCount())
                strTitle = String.format(GetResource("ApproverAgreeTabApp6"), ds.data.getCount());
                break;

            case 'E107':
                //strTitle = String.format('결재5({0})', ds.data.getCount())
                strTitle = String.format(GetResource("ApproverAgreeTabApp7"), ds.data.getCount());
                break;
        }
