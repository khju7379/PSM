/*
* SetLine.IcoClxCharger.js
* 인천 담당자
*/
var IcoClxChargerServerURL = 'IcoClxChargerServer.aspx';

var IcoClxChargerDataFields = [
	{ name: 'EmpID' },
	{ name: 'LoginID' },
	{ name: 'CellPhone' },
	{ name: 'TeamChiefYN' },
...
var leftTabNodes = $(FormXml).find[0]("select")[0];
...
for(var i=0; i<tabNodes.length; i++)
{
	...
	if(objKey.toLowerCase() == "icochargerlist")
	{
		if(src.lastIndexOf('?docCode=') > -1)
		{
			strDocCode_ico = src.substring(src.lastIndexOf('=') + 1, src.length);
		}
	}
}

//조직도에 공통적으로 들어가는 Component를 정의
GW.SetLine.IcoClxCharger = function() {
    ...
    ({
        url: IcoClxChargerServerURL,
        baseParams: { action: 'GetMemberList', docCode: strDocCode_ico },
        root: 'dataRoot',
        fields: IcoClxChargerDataFields,
        remoteSort: false
    });
    ...
    this.CurrentGridSet.IcoClxCharger.on('render', function() { this.DragDropSetting(); }, this);
};

Ext.extend(GW.SetLine.IcoClxCharger, GW.SetLine.Common, {

    ReturnSourceGrid: function() {
        return this.CurrentGridSet.IcoClxCharger;
    },
    ...
    InitGrid: function(){
		...
		([
			...
			{header: Language.DataKind + " <font color='blue'>" + DataKind + "</font>",	dataIndex: 'Display1', 	menuDisabled: true,	width: 150,	renderer: this.RendererWithTooltip},
			{header: Language.NameEmpID + "<font color='gray'>" + Position + "</font>",			dataIndex: 'Display2',	menuDisabled: true,	width: 150,	renderer: this.RendererUserName.createDelegate(this)}
...
//			{header: GetResource("UserOfficeTel"),	dataIndex: 'ExtensionNumber',	menuDisabled: true,	width: 95,		renderer: this.RendererWithTooltip}
        ...
        var gridSet =
		{
		    IcoClxCharger: this.ClxMembersGrid
		};
	...
	GetChargerTree : function(width) {
		...
		var tree = new Ext.tree.TreePanel({
			...
			loader:new Ext.tree.TreeLoader({
				url: IcoClxChargerServerURL,
				baseParams:{ action: 'GetTree', docCode: strDocCode_ico},
				requestMethod:'POST'
			}),
...
//GW.IcoClxCharger = new GW.SetLine.IcoClxCharger();
