...
// 결재모드로 동작할 최소크기
var ATTACH_MIN_CONTROL_SIZE = 182;
// 결재모드시 파일명의 크기
var ATTACH_FILENAME_SIZE = "120px";
...
(function (window, document, plupload, o, $) {
    ...
    function renderUI(obj) {
        obj.id = obj.attr('id');
        var simpleMode = obj.attr("SimpleMode").toLowerCase() === 'true';

        var width = $(obj).width();

        var Minimal = width > ATTACH_MIN_CONTROL_SIZE;

        var display = "";
        var name_width = "";

        // 결재모드일시 파일넓이 및 컨트롤 숨김처리
        if (!Minimal) {
            display = 'style="display:none;"';
            name_width = 'style="width:'+ATTACH_FILENAME_SIZE+';"';
        }
			    ...
			    '<div class="plupload_container">' +
				    ...
				    '<tr>' +
					    '<td class="plupload_cell plupload_file_name" style="background-color:white;">' +
                            '<div class="plupload_content" style="float:left;width:200px;min-height:30px;position:absolute;display:none;margin-top:33px;margin-left:10px;border:solid 1px #cccccc;border-radius:10px;background-color:white;z-index:100;">' +
					            '<div class="plupload_droptext">' + '</div>' + // + _("Drag files here.") + '</div>' +
					            '<ul class="plupload_filelist_content"> </ul>' +
					            '<div class="plupload_clearer">&nbsp;</div>' +
				            '</div>' +
						    '<div class="plupload_buttons" style="float:left;">' +
							    ...
							    '<a class="plupload_button plupload_start" style="display:none;">' + _('Upload') + '</a>&nbsp;' +
							    '<a class="plupload_button plupload_stop plupload_hidden" style="display:none;">' + _('Stop Upload') + '</a>&nbsp;' +
						    '</div>' +
					    '</td>' +
					    //'<td class="plupload_file_status" ' + display + '><span class="plupload_total_status">0%</span></td>' +
    ...
    $.widget("ui.plupload", {
        ...
        _create: function () {
            ...
            // SimpleMode 체크
            this.SimpleMode = $("#"+this.id).attr("SimpleMode").toLowerCase() == "true";
            ...
            // buttons
            this.browse_button = $('.plupload_add', this.container).attr('id', id + '_browse'); // 파일추가
            this.view_button = $('.plupload_view', this.container).attr('id', id + '_view'); // 보기 버튼(심플모드)
            this.start_button = $('.plupload_start', this.container).attr('id', id + '_start'); // 업로드 시작
            this.stop_button = $('.plupload_stop', this.container).attr('id', id + '_stop'); // 업로드 중지
        ...
        _initUploader: function () {
            ...
            for(var x=0;x<FILEUPLOAD.length;x++){
                if(this.id == FILEUPLOAD[x].id){
                    FILEUPLOAD.splice(x,1);
                }
            }
            ...
            uploader.bind('Error', function (up, err) {
                ...
                switch (err.code) {
                    ...
                    case plupload.FILE_SIZE_ERROR:
                        details = o.sprintf(_("File: %s, size: %d, max file size: %d"), err.file.name, err.file.size, plupload.parseSize(self.options.filters.max_file_size));
                        break;
            ...
            uploader.bind('PostInit', function (up) {
                ...
                self.view_button.click(function (e) {
                    //alert();
                    //$("#"+this.id.replace("_view","_dropbox")).show();
                ...
                if (!Minimal) {
                    display = 'style="display:none;"';
                    name_width = 'width:'+ATTACH_FILENAME_SIZE+';';
                }

                if(self.SimpleMode){
                    display = 'style="display:none;"';
                    name_width = 'width:160px;';
                }

                if (self.ReadMode) {
                    readmode = "display:none;";

                    $("#"+self.id).find(".plupload_filelist_footer").hide();

                    // 심플모드이고 조회모드일시 버튼영역은 보여주고 첨부추가버튼만 숨김처리한다.
                    if(self.SimpleMode){
                        $("#"+self.id).find(".plupload_filelist_footer").show();
                    }
                }
                ...
                var file_html = '<li class="plupload_file ui-state-default" id="%id%">' +
				    ...
				    '</div>' +
				    //'<div class="plupload_file_name" title="%name%" onclick="__FileDownloadLink(\'%src%\');" style="cursor:pointer;' + name_width + '" onmouseover="$(this).find(\'span\').css(\'color\',\'#F4A460\');" onmouseout="$(this).find(\'span\').css(\'color\',\'#646464\');"><span class="plupload_file_extspan"><img src="/Resources/Images/ext/%ext%.gif" onerror="this.src=\'/Resources/Images/ext/common.gif\'" alt="" /></span><span class="plupload_file_namespan">%name%</span></div>' +
                    '<div class="plupload_file_name" title="%name%" style="cursor:pointer;' + name_width + '" onmouseover="$(this).find(\'span\').css(\'color\',\'#F4A460\');" onmouseout="$(this).find(\'span\').css(\'color\',\'#646464\');"><a href="%src%"><span class="plupload_file_extspan"><img src="/Resources/Images/ext/%ext%.gif" onerror="this.src=\'/Resources/Images/ext/common.gif\'" alt="" /></span><span class="plupload_file_namespan">%name%</span></a></div>' +
				    '<div class="plupload_file_action"><div class="ui-icon ui-icon-circle-minus" onclick="__FileDelete(this,\'%unid%\');" style="cursor:pointer;' + readmode + '"> </div></div>' +
                    ...
                    $.each(_files, function (i, file) {
                        ...
                        self.filelist.append(file_html.replace(/%(\w+)%/g, function ($0, $1) {
                            if ('size' === $1) {
                                ...
                                return encodeURIComponent(file.unid);
                            } else {
                                return file[$1] || '';
                            }
                        }));
                ...
                if(window.AttAfterPostInit) window.AttAfterPostInit();
            });
            ...
            uploader.bind('UploadFile', function (up, file) {
                ...
                //file.target_name = yyyymmdd + "-" + file.name;
                //file.target_name = "123";
            ...
            uploader.bind('FileUploaded', function (up, file, result) {
                ...
                //__UploadComplete(self.id,files);
                var data = $("#" + id + "_filedata").val();

                data += file.name.replace(/\^/ig,'_') + "|" + file.size + "|" + file.type + "|" + result.response + "^";
        ...
        FileListShow : function(obj) {
            //$(".plupload_dropbox").hide(); // 전체 닫기
            $(obj).find(".plupload_dropbox").hide();
            $("#"+this.id+"_dropbox").show();
            if(obj.text().split('(').length>1){
                obj.text("Close("+obj.text().split('(')[1].replace(')','')+")");
            }
            else{
                obj.text("Close");
            }
...
//            $("#"+this.id+"_dropbox").find("#"+this.id+"_filelist").find(".plupload_file").css("border","1px solid #cccccc");
//            $("#"+this.id+"_dropbox").find("#"+this.id+"_filelist").find(".plupload_file:last").css("border","0px");
            $("#"+this.id+"_dropbox").css("height",height+"px");
            // 좌표를 검색하여 브라우져보다 클시 위치를 이동시킨다.
            var left = $("#"+this.id+"_dropbox").offset().left;
            var width = $(window).width();// 문서의 넓이
            if(width < left + 200){
                $("#"+this.id+"_dropbox").css("margin-left","-"+(left + 213 - width) + "px");
            }
            else{
                $("#"+this.id+"_dropbox").css("margin-left","10px");
            }
            //fuBoard4_filelist
        },
        ...
        FileListHide : function(obj) {
            if(obj.text().split('(').length>1){
                obj.text("View("+obj.text().split('(')[1].replace(')','')+")");
            }
            else{
                obj.text("View");
            }
        ...
        */
        ...
        _handleState: function () {
            ...
            if (up.total.queued === 0) {
                if(this.SimpleMode){
                    $('.ui-button-text', this.browse_button).html(_('Add'));
                }
                else{
                ...
                if(this.SimpleMode){
                    $('.ui-button-text', this.browse_button).html(o.sprintf(_('Add(%d)'), up.total.queued));
                }
                else{
        ...
        _addFiles: function (files) {
            ...
            if(this.SimpleMode){
			    ...
			    '</div>' +
			    '<div class="plupload_file_name" title="%name%" style="' + name_width + '"><span class="plupload_file_extspan"><img src="/Resources/Images/ext/%ext%.gif" onerror="this.src=\'/Resources/Images/ext/common.gif\'" alt="" /></span><span class="plupload_file_namespan">%name%</span></div>' +
			    ...
			    '<div class="plupload_file_thumb">' +
				    '<div class="plupload_file_dummy ui-widget-content"><span class="ui-state-disabled">%ext%</span></div>' +
			    '</div>' +
			    '<div class="plupload_file_name" title="%name%" ' + name_width + '><span class="plupload_file_extspan"><img src="/Resources/Images/ext/%ext%.gif" onerror="this.src=\'/Resources/Images/ext/common.gif\'" alt="" /></span><span class="plupload_file_namespan">%name%</span></div>' +
			    '<div class="plupload_file_action"><div class="ui-icon"> </div></div>' +
			    '<div class="plupload_file_size" ' + display + '>%size% </div>' +
			    '<div class="plupload_file_status" ' + display + '>' +
				    '<div class="plupload_file_progress ui-widget-header" style="width: 0%"> </div>' +
        ...
        Init: function () {
            ...
            var file_html = '<li class="plupload_file ui-state-default" id="%id%">' +
				...
				'</div>' +
				'<div class="plupload_file_name" title="%name%" onclick="__FileDownloadLink(\'%src%\');" style="cursor:pointer;' + name_width + '" onmouseover="$(this).find(\'span\').css(\'color\',\'#F4A460\');" onmouseout="$(this).find(\'span\').css(\'color\',\'#646464\');"><span class="plupload_file_extspan"><img src="/Resources/Images/ext/%ext%.gif" onerror="this.src=\'/Resources/Images/ext/common.gif\'" alt="" /></span><span class="plupload_file_namespan">%name%</span></div>' +
				'<div class="plupload_file_action"><div class="ui-icon ui-icon-circle-minus" onclick="__FileDelete(this,\'%unid%\');" style="cursor:pointer;"> </div></div>' +
				'<div class="plupload_file_size" ' + display + '>%size% </div>' +
				'<div class="plupload_file_status" ' + display + '>' +
					'<div class="plupload_file_progress ui-widget-header" style="width: 0%"> </div>' +
					'<span class="plupload_file_percent">%percent% </span>' +
            ...
            if (data != "") {
                ...
                // 심플모드일시 갯수만큼 View화면에 적용
                if(cnt>0 && this.SimpleMode){
                    this.view_button.find(".ui-button-text").text("View" + "(" + cnt + ")");
                }
            }
