    ...
    public class PSM4070 : BizBase
    {
        ...
        public DataSet UP_GET_WORKORDER_LIST(Hashtable ht)
        {
            using (DB2Connecter dbCon = new DB2Connecter("DB2"))
            {
                ...
                return dbCon.ExcuteDataSet("PSSCMLIB.PSM_PSM4070_WORKORDER_LIST");
            }
        }
        #endregion
        ...
        #region Description : 가동전 점검표 검점내용 좌측 메뉴 리스트 조회
        public DataSet UP_GET_EXAM_CHECK_MENU_LIST(Hashtable ht)
        {
            using (DB2Connecter dbCon = new DB2Connecter("DB2"))
            {
                ...
                return dbCon.ExcuteDataSet("PSSCMLIB.PSM_PSM4071_EXAM_CHECK_MENU_LIST");
            }
        }
        #endregion

        #region Description : 가동전 점검표 검점내용 리스트 조회
        public DataSet UP_GET_EXAM_CHECK_CONTENTS_LIST(Hashtable ht)
        {
            using (DB2Connecter dbCon = new DB2Connecter("DB2"))
            {   
                dbCon.AddParameters("P_PECCGUBUN", DbType.String, ht["P_PECCGUBUN"]);
                ...
                return dbCon.ExcuteDataSet("PSSCMLIB.PSM_PSM4071_EXAM_CHECK_CONTENTS_LIST");
            }
        }
        #endregion

        #region Description : 가동전 점검표 검점내용 등록
        public DataSet UP_EXAM_CHECK_CONTENTS_ADD(Hashtable ht, Hashtable[] hts)
        {
            string sPECCSEQ = string.Empty;

            DataSet ds = new DataSet();
            DataTable dt = new DataTable();
            DataRow row;

            string sMSG = UP_EXAM_CHECK_CONTENTS_Check(hts);
            ...
            {
                // 순번 가져오기
                if (hts[0]["P_PECCSEQ"].ToString() == "")
                {
                    using (DB2Connecter dbCon = new DB2Connecter("DB2"))
                    {
                        dbCon.AddParameters("P_PECCGUBUN", DbType.String, hts[0]["P_PECCGUBUN"]);
                        dbCon.AddParameters("P_PECCDATE", DbType.String, hts[0]["P_PECCDATE"]);

                        ds = dbCon.ExcuteDataSet("PSSCMLIB.PSM_PSM4071_EXAM_CHECK_CONTENTS_SEQ");

                        if (ds.Tables[0].Rows.Count > 0)
                        {
                            sPECCSEQ = ds.Tables[0].Rows[0]["PECCSEQ"].ToString();
                        }
                    }
                }
                else
                {
                    sPECCSEQ = hts[0]["P_PECCSEQ"].ToString();
                    ...
                    using (DB2Connecter dbCon = new DB2Connecter("DB2"))
                    {
                        dbCon.AddParameters("P_PECCGUBUN", DbType.String, hts[0]["P_PECCGUBUN"]);
                        dbCon.AddParameters("P_PECCDATE", DbType.String, hts[0]["P_PECCDATE"]);
                        dbCon.AddParameters("P_PECCSEQ", DbType.String, sPECCSEQ);

                        dbCon.ExcuteNonQuery("PSSCMLIB.PSM_PSM4071_EXAM_CHECK_CONTENTS_DEL");
                    }
                }
                ...
                using (DB2Connecter dbCon = new DB2Connecter("DB2"))
                {
                    ...
                    {
                        dbCon.AddParameters("P_PECCGUBUN", DbType.String, hts[i]["P_PECCGUBUN"]);
                        dbCon.AddParameters("P_PECCDATE", DbType.String, hts[i]["P_PECCDATE"]);
                        dbCon.AddParameters("P_PECCSEQ", DbType.VarNumeric, sPECCSEQ);
                        dbCon.AddParameters("P_PECCNUM", DbType.VarNumeric, hts[i]["P_PECCNUM"]);
                        dbCon.AddParameters("P_PECCDESC", DbType.String, hts[i]["P_PECCDESC"]);
                        dbCon.AddParameters("P_PECLEVEL", DbType.String, hts[i]["P_PECLEVEL"]);
                        dbCon.AddParameters("P_PECCCHECK", DbType.String, hts[i]["P_PECCCHECK"]);
                        dbCon.AddParameters("P_PECCHISAB", DbType.String, Document.UserInfo.EmpID);

                        dbCon.ExcuteNonQuery("PSSCMLIB.PSM_PSM4071_EXAM_CHECK_CONTENTS_ADD");
                    }
        ...
        #region Description : 가동전 점검표 검점내용 삭제
        public DataSet UP_EXAM_CHECK_CONTENTS_DEL(Hashtable ht, Hashtable[] hts)
        {
            DataSet ds = new DataSet();
            DataTable dt = new DataTable();
            DataRow row;
            ...
            if (sMSG == "")
            {
                ...
                using (DB2Connecter dbCon = new DB2Connecter("DB2"))
                {
                    dbCon.AddParameters("P_PECCGUBUN", DbType.String, hts[0]["P_PECCGUBUN"]);
                    dbCon.AddParameters("P_PECCDATE", DbType.String, hts[0]["P_PECCDATE"]);
                    dbCon.AddParameters("P_PECCSEQ", DbType.String, hts[0]["P_PECCSEQ"]);
        ...
        #region Description : 가동전 점검표 검점내용 등록/삭제 체크
        public string UP_EXAM_CHECK_CONTENTS_Check(Hashtable[] hts)
        {
            ...
            if (hts[0]["P_PECCSEQ"].ToString() != "")
            {
                ...
                using (DB2Connecter dbCon = new DB2Connecter("DB2"))
                {
                    dbCon.AddParameters("P_PECSGUBUN", DbType.String, hts[0]["P_PECCGUBUN"].ToString());
                    dbCon.AddParameters("P_PECSDATE", DbType.String, hts[0]["P_PECCDATE"].ToString());
                    dbCon.AddParameters("P_PECSSEQ", DbType.VarNumeric, hts[0]["P_PECCSEQ"].ToString());

                    ds = dbCon.ExcuteDataSet("PSSCMLIB.PSM_PSM4071_EXAM_CHECK_CHECK");

                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        sMSG = "가동전 점검표 자료가 존재합니다.";
        ...
        #region Description : 가동전 점검표 검점내용 복사
        public DataSet UP_EXAM_CHECK_CONTENTS_COPY(Hashtable ht)
        {
            string sPECCSEQ = string.Empty;
            ...
            using (DB2Connecter dbCon = new DB2Connecter("DB2"))
            {
                dbCon.AddParameters("P_PECCGUBUN", DbType.String, ht["P_COPYPECCGUBUN"]);
                dbCon.AddParameters("P_PECCDATE", DbType.String, ht["P_COPYPECCDATE"]);
            ...
            using (DB2Connecter dbCon = new DB2Connecter("DB2"))
            {
                ...
                dbCon.AddParameters("P_PECCDATE", DbType.String, ht["P_PECCDATE"]);
                dbCon.AddParameters("P_PECCSEQ", DbType.VarNumeric, ht["P_PECCSEQ"]);
                dbCon.AddParameters("P_COPYPECCGUBUN", DbType.String, ht["P_COPYPECCGUBUN"]);
                dbCon.AddParameters("P_COPYPECCDATE", DbType.String, ht["P_COPYPECCDATE"]);
                dbCon.AddParameters("P_COPYPECCSEQ", DbType.VarNumeric, sPECCSEQ);
                dbCon.AddParameters("P_PECCHISAB", DbType.String, Document.UserInfo.EmpID);
                
                dbCon.ExcuteNonQuery("PSSCMLIB.PSM_PSM4072_EXAM_CHECK_CONTENTS_COPY");
            }
        ...
        #region Description : 가동전 점검표 점거자 조회
        public DataSet UP_GET_EXAM_INSPECT_LIST(Hashtable ht)
        {
            DataSet ds = new DataSet();
            ...
            if (ht["P_WKGUBUN"].ToString() == "NEW")
            {
                using (DB2Connecter dbCon = new DB2Connecter("DB2"))
                {
                    ...
                    ds = dbCon.ExcuteDataSet("PSSCMLIB.PSM_PSM4073_EXAM_INSPECT_LASTLIST");
                }
            }
            else { 
                using (DB2Connecter dbCon = new DB2Connecter("DB2"))
                {
                    dbCon.AddParameters("P_PEICTEAM", DbType.String, ht["P_PEICTEAM"]);
                    dbCon.AddParameters("P_PEICDATE", DbType.String, ht["P_PEICDATE"]);
                    dbCon.AddParameters("P_PEICSEQ", DbType.VarNumeric, ht["P_PEICSEQ"]);

                    ds = dbCon.ExcuteDataSet("PSSCMLIB.PSM_PSM4073_EXAM_INSPECT_LIST");
                }
            }
        ...
        public DataSet UP_EXAM_CHECK_PRT(Hashtable ht)
        {
            using (DB2Connecter dbCon = new DB2Connecter("DB2"))
            {
                ...
                return dbCon.ExcuteDataSet("PSSCMLIB.PSM_PSM4073_EXAM_CHECK_PRT");
            }
        }
        #endregion
        ...
        #region Description : 가동전 점검표 점검자 등록
        public DataSet UP_EXAM_INSPECT_ADD(Hashtable ht, Hashtable[] hts)
        {
            DataSet ds = new DataSet();
            DataTable dt = new DataTable();
            DataRow row;
            ...
            using (DB2Connecter dbCon = new DB2Connecter("DB2"))
            {
                for (int i = 0; i < hts.Length; i++)
                {
                    dbCon.AddParameters("P_PEICTEAM", DbType.String, hts[i]["P_PEICTEAM"]);
                    dbCon.AddParameters("P_PEICDATE", DbType.String, hts[i]["P_PEICDATE"]);
                    dbCon.AddParameters("P_PEICSEQ", DbType.VarNumeric, hts[i]["P_PEICSEQ"]);
                    dbCon.AddParameters("P_PEINUM", DbType.VarNumeric, hts[i]["P_PEINUM"]);
                    dbCon.AddParameters("P_PEISABUN", DbType.String, hts[i]["P_PEISABUN"]);
                    dbCon.AddParameters("P_PEICHKDATE", DbType.String, hts[i]["P_PEICHKDATE"]);
                    dbCon.AddParameters("P_PEIHISAB", DbType.String, Document.UserInfo.EmpID);
                    dbCon.AddParameters("P_GUBUN", DbType.String, hts[i]["P_GUBUN"]);

                    dbCon.ExcuteNonQuery("PSSCMLIB.PSM_PSM4073_EXAM_INSPECT_ADD");
                }
        ...
        #region Description : 점검요청 메일 발송
        public DataSet UP_Mail_Inspect_Send(Hashtable ht)
        {
            ...
            string sSENDER = TaeYoung.Biz.Document.UserInfo.LoginID;
            string sPGURL = string.Empty;
            string sPECTITLE = string.Empty;
            string sPECWOTEAM = string.Empty;
            string sPECWODATE = string.Empty;
            string sPECWOSEQ = string.Empty;
            string sPECTEAM = string.Empty;
            string sPECDATE = string.Empty;
            string sPECSEQ = string.Empty;
            ...
            {   
                ...
                sPECWOTEAM = ht["P_PECWOTEAM"].ToString();
                sPECWODATE = ht["P_PECWODATE"].ToString();
                sPECWOSEQ = ht["P_PECWOSEQ"].ToString();
                sPECTEAM = ht["P_PECTEAM"].ToString();
                sPECDATE = ht["P_PECDATE"].ToString();
                sPECSEQ = ht["P_PECSEQ"].ToString();

                sPGURL = "/Portal/PSM/PSM40/PSM4073.aspx?";
                sPGURL = sPGURL + "param=UPT&param1=";
                sPGURL = sPGURL + sPECWOTEAM + "-" + sPECWODATE + "-" + sPECWOSEQ + "^" + sPECTEAM + "-" + sPECDATE + "-" + sPECSEQ;

                using (DB2Connecter dbCon = new DB2Connecter("DB2"))
                {
                    // 점검자 메일 계정 가져오기
                    dbCon.AddParameters("P_PEICTEAM", DbType.String, sPECTEAM.ToString());
                    dbCon.AddParameters("P_PEICDATE", DbType.String, sPECDATE.ToString());
                    dbCon.AddParameters("P_PEICSEQ", DbType.VarNumeric, sPECSEQ.ToString());

                    DataSet ds1 = dbCon.ExcuteDataSet("PSSCMLIB.PSM_PSM4073_EXAM_INSPECT_NEXTAPPR");
                    ...
                    if (ds1.Tables[0].Rows.Count > 0)
                    {
                        ...
                        {   
                            sMailTo = ds1.Tables[0].Rows[i]["KBMAILID"].ToString() + "@taeyoung.co.kr";

                            // url 연결 수정필요
                            sUrl = "http://psm.taeyoung.co.kr/Portal/SitePostUrl.aspx?url=http://psm.taeyoung.co.kr/Portal/PortalLogin.aspx&ACTION=APPPAGE&SABUN=" + ds1.Tables[0].Rows[i]["PEISABUN"].ToString() + "&ID=" + ds1.Tables[0].Rows[i]["PEISABUN"].ToString() + "&LINKURL=" + sPGURL;
                            ...
                            sTitle = sPECTITLE.ToString() + " - 점검요청";
                            ...
                            try
                            {
                                ...
                                sLine += "    <tbody> ";
                                sLine += "        <tr> ";
                                sLine += "            <th colspan = 2 style = 'font-family:맑은 고딕,Verdana,sans-serif;height:50px;font-weight:bold;background:#ab2457; font-size:18px;color:#FFFFFF;text-align:center; vertical-align:middle;'> 가동전 점검 </th> ";
                                ...
                                sLine += "            <td style = 'font-family:맑은 고딕,Verdana,sans-serif;padding:10px;border-bottom:1px solid #EAEAEA;'> " + sPECWOTEAM + "-" + sPECWODATE + "-" + sPECWOSEQ + "</td> ";
                                ...
                                sLine += "            <td style = 'font-family:맑은 고딕,Verdana,sans-serif;padding:10px;border-bottom:1px solid #EAEAEA;'> " + sPECTEAM + "-" + sPECDATE + "-" + sPECSEQ + "</td> ";
                                sLine += "            <td style = 'width:80px;border-bottom:1px solid #EAEAEA;'> &nbsp;</td> ";
                                sLine += "        </tr> ";

                                sLine += "        <tr> ";
                                sLine += "            <th style = 'font-family:맑은 고딕,Verdana,sans-serif;width:120px;padding:10px 10px 10px 30px;;border-bottom:1px solid #EAEAEA;text-align:left; background:#DEDEEF;'> 제목 </th> ";
                                sLine += "            <td style = 'font-family:맑은 고딕,Verdana,sans-serif;padding:10px;border-bottom:1px solid #EAEAEA;'> " + sTitle + "</td> ";
                                sLine += "            <td style = 'width:80px;border-bottom:1px solid #EAEAEA;'> &nbsp;</td> ";
                                sLine += "        </tr> ";
                    ...
                    else
                    {
                        ...
                        ds1 = dbCon.ExcuteDataSet("PSSCMLIB.PSM_GET_MAILID_LIST");

                        if (ds1.Tables[0].Rows.Count > 0)
                        {
                            sMailTo = ds1.Tables[0].Rows[0]["KBMAILID"].ToString() + "@taeyoung.co.kr";
                        }


                        // url 연결 수정필요
                        sUrl = "http://psm.taeyoung.co.kr/Portal/SitePostUrl.aspx?url=http://psm.taeyoung.co.kr/Portal/PortalLogin.aspx&ACTION=APPPAGE&SABUN=" + ht["P_SENDER"].ToString() + "&ID=" + ht["P_SENDER"].ToString() + "&LINKURL=" + sPGURL;
                        ...
                        sTitle = sPECTITLE.ToString() + " - 점검완료";
        ...
        private DataSet UP_RETRACT_Mail_Send(Hashtable ht)
        {
            ...
            using (DB2Connecter dbCon = new DB2Connecter("DB2"))
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    sTITLE = "[결재철회/가동전 점검] - " + sPECTITLE.ToString();
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        for (i = 0; i < ds.Tables[0].Rows.Count; i++)
                        {
                            ...
                            sTitle = sWOWORKTITLE.ToString() + " - 가동전 점검 완료";
